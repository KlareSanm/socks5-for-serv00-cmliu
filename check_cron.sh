...
CRON_JOB="0 3 * * 1 $PM2_PATH resurrect >> /home/$(whoami)/pm2_resurrect.log 2>&1"
...
if [ "$(command -v pm2)" == "/home/${USER}/.npm-global/bin/pm2" ]; then
  ...
  if [ -e "${WORKDIR}/start.sh" ] && [ -e "${FILE_PATH}/config.json" ]; then
    ...
    (crontab -l | grep -F "* * pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") || (crontab -l; echo "0 3 * * 1 pgrep -x \"nezha-agent\" > /dev/null || ${CRON_NEZHA}") | crontab -
    (crontab -l | grep -F "* * pgrep -x \"s5\" > /dev/null || ${CRON_S5}") || (crontab -l; echo "0 3 * * 1 pgrep -x \"s5\" > /dev/null || ${CRON_S5}") | crontab -
